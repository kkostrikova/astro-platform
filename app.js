...
function injectAuthUI(){
  ...
  style.textContent=`
    .auth-gate-wrap{padding-top:1.2rem;padding-bottom:1.2rem}.auth-gate-card{max-width:720px;margin:0 auto;padding:clamp(22px,4vw,42px);text-align:left}.auth-gate-card h3{margin:.25rem 0 .55rem;font-family:'Cormorant Garamond',serif;font-size:clamp(28px,4vw,42px);font-weight:500}.auth-gate-card>p{max-width:620px;color:var(--muted,#736f69);line-height:1.65}.auth-form-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px}.auth-form-grid label{display:grid;gap:7px;font-size:13px}.auth-form-grid input{width:100%}.auth-name-field{grid-column:1/-1}.auth-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.auth-meta{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:14px;font-size:12px;color:var(--muted,#736f69)}.auth-dot{width:7px;height:7px;border-radius:999px;background:currentColor;opacity:.5}.auth-status-good{color:#476a52}.auth-status-wait{color:#8a6a32}.auth-gate-wrap[hidden],.auth-form-grid[hidden],.auth-actions[hidden],.supabase-private[hidden],#logoutBtn[hidden]{display:none!important}.panel-title-actions{display:flex;align-items:center;gap:8px}.delete-client-btn{color:#9a4d4d}.delete-client-btn:disabled{opacity:.3;cursor:not-allowed}@media(max-width:680px){.auth-form-grid{grid-template-columns:1fr}.auth-name-field{grid-column:auto}.auth-gate-card{padding:20px}.auth-actions .btn{flex:1 1 auto}}
  `;
...
function authGateHtml(kind,title,text){const nameField=kind==='client'?`<label class="auth-name-field">Ім’я<input id="${kind}AuthName" type="text" autocomplete="name" placeholder="Як до вас звертатися"></label>`:'';return `<section class="glass panel auth-gate-card"><div class="eyebrow">ПРИВАТНИЙ ДОСТУП</div><h3 id="${kind}AuthTitle">${title}</h3><p id="${kind}AuthText">${text}</p><div class="auth-form-grid" id="${kind}AuthForm">${nameField}<label>Email<input id="${kind}AuthEmail" type="email" autocomplete="email" placeholder="name@example.com"></label><label>Пароль<input id="${kind}AuthPassword" type="password" autocomplete="current-password" placeholder="Ваш пароль"></label></div><div class="auth-actions" id="${kind}AuthActions"><button class="btn primary" data-auth-login="${kind}">Увійти</button><button class="btn ghost" data-auth-signup="${kind}">Створити акаунт</button></div><div class="auth-meta"><span class="auth-dot"></span><span id="${kind}AuthStatus">Supabase Auth • приватна сесія</span></div></section>`}
...
function updateAccessUI(){
  ...
  if(!currentUser){
    setPrivate('client',false);setGate('client',{title:'Кабінет клієнта',text:'Увійдіть або створіть акаунт. Після підтвердження email ваш клієнтський профіль створиться автоматично.'});
  ...
  if(isAstrologer){
    ...
    if(clientsCache.length)setPrivate('client',true);else{setPrivate('client',false);setGate('client',{title:'Поки немає клієнтів',text:'Після реєстрації клієнти автоматично з’являтимуться у вашій базі.',status:'Акаунт астролога активний',showForm:false,statusClass:'auth-status-good'})}
  }else{
    setPrivate('astro',false);setGate('astro',{title:'Кабінет астролога не активований',text:'Цей акаунт не має прав астролога. Кабінет Вікторії доступний лише її акаунту.',status:`Увійшли як ${currentUser.email||''}`,showForm:false,statusClass:'auth-status-wait'});
    if(clientsCache.length)setPrivate('client',true);else{setPrivate('client',false);setGate('client',{title:'Профіль готується',text:'Клієнтська картка створюється автоматично після підтвердження email. Якщо ви щойно підтвердили адресу, оновіть сторінку.',status:`Увійшли як ${currentUser.email||''}`,showForm:false,statusClass:'auth-status-good'})}
  }
}
...
async function authSignup(kind){if(!sb)return toast('Підключення ще ініціалізується');const email=$('#'+kind+'AuthEmail')?.value.trim().toLowerCase(),password=$('#'+kind+'AuthPassword')?.value||'',name=kind==='client'?($('#'+kind+'AuthName')?.value.trim()||''):'';if(kind==='client'&&!name)return toast('Вкажіть ім’я');if(!email||!password)return toast('Вкажіть email і пароль');if(password.length<8)return toast('Пароль має містити щонайменше 8 символів');const payload={email,password};if(kind==='client')payload.options={data:{name}};const {data,error}=await sb.auth.signUp(payload);if(error)return toast(friendlyError(error));if(data.session)toast('Акаунт створено і вхід виконано');else toast('Акаунт створено. Підтвердіть email і поверніться на платформу')}
